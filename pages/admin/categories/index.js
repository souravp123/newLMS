...
const Index = ({ user }) => {
	...
	const fetchData = async () => {
		...
		try {
			...
			const response = await axios.get(
				`${baseUrl}/api/categories/delcategory`,
				payload
			);
	...
	const isEnrolled = (catId) => {
		const category = categories.find(cat => cat.id === catId);
		if (category) {
			return category.courses.some(course => course.enrolments.length > 0);
		}
		return false;
	};
	...
	const handleDelete = async (catId) => {
		try {
			const category = categories.find(cat => cat.id === catId);
			if (category) {
				const hasEnrolledCourses = category.courses.some(course => course.enrolments.length > 0);
				if (hasEnrolledCourses) {
					toast.error("Cannot delete category becouse courses are enrolled with this category", {
						style: {
							border: "1px solid #ff0033",
							padding: "16px",
							color: "#ff0033",
